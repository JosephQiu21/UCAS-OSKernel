...
static void write_os_size(int nbytes, FILE * img, int offset);
...
static void create_image(int nfiles, char *files[])
{
    ...
    int is_bootblock = 1;
    int os_size_offset = 0; // Offset from OS_SIZE_LOC for each kernel
    FILE *fp, *img;
    Elf64_Ehdr ehdr;
    Elf64_Phdr phdr;
...
static void write_os_size(int nbytes, FILE * img, int offset)
{
    ...
    fseek(img, OS_SIZE_LOC - offset, SEEK_SET);
